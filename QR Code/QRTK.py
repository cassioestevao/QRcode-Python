...
class GeradorQRCode(tk.Frame):
    def __init__(self, master=None):
        ...
        self.master = master
        self.master.title("Gerador de QR Code")
        
        self.texto_qrcode = tk.StringVar()
        self.criar_widgets()

    def criar_widgets(self):
        self.label = tk.Label(self.master, text="Digite o texto que deseja codificar:")
        self.label.pack()

        self.campo_texto = tk.Entry(self.master, textvariable=self.texto_qrcode)
        self.campo_texto.pack()

        self.botao = tk.Button(self.master, text="Gerar QR Code", command=self.gerar_qrcode)
        self.botao.pack()
